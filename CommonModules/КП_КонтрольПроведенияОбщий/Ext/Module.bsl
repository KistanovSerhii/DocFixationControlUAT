...
Функция ТехКартыЗУПСодержатВидРаботВГоду(мВидыРаботКод, ГодТехКарты) Экспорт
	...
	СвернутыйМВидыРаботКод	= СвернутьМассив(мВидыРаботКод);
	
	ПараметрыЗапроса.Вставить("ВидРаботКод", СвернутыйМВидыРаботКод);
	ПараметрыЗапроса.Вставить("ГодТехКарты", ГодТехКарты);	
	
	ОтветЗапросаWS 	= КП_ОбменДаннымиWS.ПослатьЗапросWS(НовыйНастройкиПрокси, ТекстЗапроса, ПараметрыЗапроса);		
	Ответ			= НовыйОтветЗапросаWS();
	
	Если ОтветЗапросаWS = Неопределено Тогда
		
		Ответ.Отказ = Истина;
		Ответ.Данные.Добавить("Нет подключения к ЗУП! Проверка разрешения провести документ не выполнена!");
	...
	Для Каждого ВидРаботКод Из СвернутыйМВидыРаботКод Цикл
		найден = ОтветЗапросаWS.Найти(ВидРаботКод, "ВидРаботКод");
		
		Если найден <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		Ответ.Данные.Добавить(ВидРаботКод);		
	КонецЦикла;
	
	Ответ.Отказ = Ответ.Данные.Количество() > 0;
...
// Функция - Новый ответ запроса WS
// 
// Возвращаемое значение:
//  ОтветЗапросаWS - Структура ИЗ
//		*Отказ  - Булево
//		*Данные - Массив
//
Функция НовыйОтветЗапросаWS()

	ОтветЗапросаWS = Новый Структура;
	
	ОтветЗапросаWS.Вставить("Отказ");
	ОтветЗапросаWS.Вставить("Данные", Новый Массив);
	
	Возврат ОтветЗапросаWS;
